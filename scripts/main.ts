...
world.beforeEvents.playerBreakBlock.subscribe(({ block, dimension, itemStack, player }) => {
  ...
  if (player.isSneaking && player.getTags().includes(`TreeActive`)) activation.treeCapitatorActivate({ block, dimension, itemStack, player });
  if (player.isSneaking && player.getTags().includes(`VeinActive`)) activation.veinMinerActivate({ block, dimension, itemStack, player });
});
