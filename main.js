// scripts/main.js
import { Entity as Entity2, Player as Player3, world as world3, ItemStack as ItemStack2, Dimension } from "@minecraft/server";

// scripts/keyyard/processor.js
import {
  system,
  ItemStack,
...
var BlockProcessor = class _BlockProcessor {
  static processBlock(player, block, item, isRecursive, blockData2, unbreakingLevel, fortuneLevel, hasSilkTouch) {
    ...
    if (isRecursive && _BlockProcessor.shouldContinueProcessing(unbreakingLevel)) {
      ...
      currentLevel++;
      const durabilityComponent2 = item.getComponent(ItemDurabilityComponent.componentId);
      if (currentLevel >= durabilityComponent2.maxDurability) {
        return;
      } else {
        system.runTimeout(() => {
      ...
      function* process() {
        ...
        do {
          element = array.shift();
          filterOtherBlock2(element.above());
          filterOtherBlock2(element.south());
          filterOtherBlock2(element.north());
          filterOtherBlock2(element.east());
          filterOtherBlock2(element.west());
...
// scripts/keyyard/activation.js
import { Entity, Player, system as system2, world, EquipmentSlot, EntityEquippableComponent } from "@minecraft/server";
...
// scripts/joins/index.ts
import { system as system3, world as world2 } from "@minecraft/server";
function firstJoinObjective() {
  const key = "keyyard:first_join";
  return world2.scoreboard.getObjective(key) ?? world2.scoreboard.addObjective(key);
}
function onFirstJoin(player) {
  ...
  system3.runTimeout(() => {
    player.sendMessage("<Keyyard> To use Vein Miner, hold a pickaxe and break a block while sneaking.");
  }, 350);
  system3.runTimeout(() => {
    player.sendMessage(
  ...
  system3.runTimeout(() => {
    player.sendMessage("<Keyyard> Enjoy the game!");
    world2.scoreboard.getObjective("fg_shm_first_join")?.setScore("first_join", 1);
  }, 650);
}
function getScore(obj, e) {
  return obj.hasParticipant(e) ? obj.getScore(e) : obj.addScore(e, 0);
}
world2.afterEvents.playerSpawn.subscribe(({ initialSpawn, player }) => {
  const objective = firstJoinObjective();
  if (initialSpawn && getScore(objective, "first") !== 1) {
    objective.setScore("first", 1);
    onFirstJoin(player);
  }
});
